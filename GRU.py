...
# design network
model = Sequential()
model.add(GRU(50, input_shape=(train_X.shape[1], train_X.shape[2]), bias_constraint=True))
model.add(Dense(1))

model.compile(loss='mean_squared_error', optimizer='sgd')
# fit network
history = model.fit(train_X, train_y, epochs=50, batch_size=72, validation_data=(val_X, val_y), verbose=2,
                    shuffle=False)
